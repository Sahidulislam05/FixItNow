...
import { ICreateServicePayload, IServiceQuery, IUpdateServicePayload } from "./service.interface";
...
const updateMyServiceInDB = async (serviceId: string, technicianId: string, payload: IUpdateServicePayload) => {
    const service = await prisma.service.findUniqueOrThrow({
        where: { id: serviceId }
    });
...
// hard delete না করে soft delete — কারণ পুরনো Booking এই service কে
// রেফার করতে পারে (Booking.serviceId), hard delete করলে booking history হারিয়ে যাবে
const deleteMyServiceFromDB = async (serviceId: string, technicianId: string) => {
    const service = await prisma.service.findUniqueOrThrow({
        where: { id: serviceId }
    });
